...
def get_clever_scores(model, dataloader, **args):
  cl_scores = []
  model.eval()
  for data in tqdm(dataloader):
      clever_dis = clever_score(model, data[0][0], **args)
      cl_scores.append(np.round(clever_dis, 3))
...
def labels_to_oneh(labels, num_classes=10):
  oneh = torch.nn.functional.one_hot(labels, num_classes=num_classes)
  return oneh.cpu().detach().numpy()

def brier_score(y, probs, axis=None):
  ...
  """
  if axis is None:
    return np.mean(np.power(probs - y, 2))
  else:
    return np.mean(np.power(probs - y, 2), axis)
...
def quadratic_score(y, probs, axis=None):
  if axis is None:
    return 1 - np.sum(np.power(probs, 2))
  else:
    return 1 - np.sum(np.power(probs, 2), axis)

def compute_entropy(probs, axis=None):
    probs[probs == 0] = 1e-8
    log_probs = np.log(probs)
    p_log_p = log_probs * probs
    if axis:
      entropy = -np.nansum(p_log_p, axis)
    else:
